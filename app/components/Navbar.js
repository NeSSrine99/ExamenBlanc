  ...
  return (
    <nav className="flex items-center justify-between px-4 py-3 bg-white shadow-md relative">
      ...
      <div className="hidden md:flex items-center gap-6 font-medium text-secondary">
        {["/accueil", "/annonces", "/publier", "/inscription"].map(
          (path, index) => {
            ...
            return (
              <Link
                key={index}
                href={path}
                className="font-medium text-secondary  hover:text-primary hover:underline transition duration-300"
              >
                {names[index]}
              </Link>
            );
      ...
      >
        <Link
          href="/accueil"
          className="font-medium text-secondary hover:text-primary hover:underline transition duration-300 w-full"
        >
          Accueil
        </Link>
        <Link
